...
void PythonProcessor::handleSpike (SpikePtr spike)
{
    if (spike->getStreamId() == currentStream)
    {
        ...
        {
            pyObject->attr ("handle_spike") (sourceNodeId, electrodeName.toRawUTF8(), numChans, numSamples, sampleNum, sortedId, spikeData);
        }
    }
}
...
bool PythonProcessor::initInterpreter (String pythonHome)
{
    ...
    try
    {
        ...
        // Set Python home
        status = PyConfig_SetString (&config, &config.home, targetFolder.getFullPathName().toWideCharPointer());
        ...
        py::initialize_interpreter (&config);
        {
            ...
            if (Py_IsInitialized() > 0)
            {
                ...
                os.attr ("add_dll_directory") (targetFolder.getChildFile ("Library\\bin").getFullPathName().toWideCharPointer());
#endif
            }
